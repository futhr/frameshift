...
| The packaged macOS app opens the same authoritative shell controls from its Dock entry | Swift shell/package | `scripts/check macos` verifies bundle metadata, app launch, and the packaged IPC path; a live desktop reopen check remains required | The Dock window shares the menu panel's `ShellModel`; desktop reopen evidence remains open |
