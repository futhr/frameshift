...
| A durable normalized master is read back with object and package verification, renders once, reuses the artifact cache, queues, and converges in-process | Core/renderer/simulator | `Frameshift.LibraryTest`, `Frameshift.RenderPipelineTest` | Component evidence only; installed render/send and real authenticated frame transport remain open |
