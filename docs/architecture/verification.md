...
| Each accepted render and delivery intent pins an asset-specific work digest and a reusable admitted renderer/profile/transfer qualification through restart, switching, and rollback | Core/renderer/delivery | `Frameshift.Qualification.IdentityTest` and `Frameshift.Qualification.ProfileTest` cover canonical strict identity and byte-affecting profile changes; `Frameshift.RendererTest` checks the startup executable digest. Producer fixtures, migration, pin/restart, incompatible profile, duplicate transfer, ambiguous effect, and rollback tests required | Pure versioned binding, work, result, and profile identities and a bounded startup renderer fingerprint are implemented; durable qualification admission, exact launched-process attestation, workflow pinning, cohort promotion, and physical qualification are open |
