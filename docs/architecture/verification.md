...
| Each accepted render and delivery intent pins an asset-specific work digest and a reusable admitted renderer/profile/transfer qualification through restart, switching, and rollback | Core/renderer/delivery | `Frameshift.Qualification.IdentityTest`, `Frameshift.Qualification.ProfileTest`, `Frameshift.Qualification.StoreTest`, `Frameshift.RenderPipelineTest`, and `Frameshift.RendererTest` cover canonical identity, profile changes, durable candidate/admission/activation, accepted work/result, restart, switch, rollback, exact build refusal, and qualified render/cache replay. Producer fixtures, legacy migration, delivery pin/restart, duplicate transfer, and ambiguous effect tests required | Pure identities, startup renderer fingerprint, durable binding decisions, atomic accepted work/result custody, and an opt-in qualified render path are implemented; exact launched-process attestation, delivery pinning, production admission evidence, cohort promotion, and physical qualification are open |
| The packaged macOS app opens the same authoritative shell controls from its Dock entry and accepts multiline instructions | Swift shell/package | `scripts/check macos` verifies bundle metadata, app launch, a newline-bearing packaged IPC round trip, and a Swift shell model test; a live AX check entered two lines with Return | The Dock window shares the menu panel's `ShellModel`; native multiline editing and saved-newline readback pass. Desktop reopen evidence remains open |
