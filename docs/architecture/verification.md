...
| Each accepted render and delivery intent pins an asset-specific work digest and a reusable admitted renderer/profile/transfer qualification through restart, switching, and rollback | Core/renderer/delivery | `Frameshift.Qualification.IdentityTest`, `Frameshift.Qualification.ProfileTest`, and `Frameshift.Qualification.StoreTest` cover canonical identity, profile changes, durable candidate/admission/activation, replay, restart, switch, rollback, and forgotten-frame refusal; `Frameshift.RendererTest` checks the startup executable digest. Producer fixtures, migration, work pin/restart, duplicate transfer, and ambiguous effect tests required | Pure identities, startup renderer fingerprint, and durable single-writer qualification decisions are implemented; exact launched-process attestation, workflow pinning, cohort promotion, and physical qualification are open |
