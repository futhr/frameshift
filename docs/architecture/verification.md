...
| --- | --- | --- | --- |
| Portable domain boundaries preserve delivery, content-reference, replay, and audit atomicity under one writer | Core | Transition characterization, crash/restart, contention, and Mac/Linux contract tests required | Specified in the domain map; extraction and cross-platform qualification remain open |
| A failed update is traceable by command and attempt ID across durable audit, native logs, metrics, and authoritative display state | Core/shell | Packaged command-to-display failure injection, redaction, restart, and correlation tests required | Specified in the diagnostics contract; instrumentation and end-to-end evidence remain open |
| Console.app and `/usr/bin/log` read core and shell operational logs without a Frameshift UI; health, metrics, and audit have a bounded read-only CLI | Core/shell | Installed log-bridge, peer-authentication, helper crash, pagination, and size-bound tests required | Specified; current core stdout/stderr are discarded by the bundled launcher |
| Local metric catalog preserves units, bounded dimensions, restart coverage, and disk/resource ceilings | Core | Reporter unit tests, long-running load and idle measurements on Mac and Linux required | Specified; no first-class metric reporter exists yet |
