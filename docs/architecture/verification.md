...
| A physical bootstrap record contains only a device ID, frame SPKI pin, and at least 16 secret bytes; the host checks the presented peer certificate before sending the secret and later matches the authenticated TD device ID | Pairing/core | `Frameshift.Pairing.BootstrapTest`, `Frameshift.Pairing.RequestTest`, `Frameshift.Pairing.WindowTest`, `Frameshift.Pairing.EndpointTest`, `Frameshift.Pairing.ClientTest`, `Frameshift.Pairing.AdmissionTest`, `Frameshift.Pairing.AdmissionLiveTest`, `Frameshift.Pairing.StoreTest`, `Frameshift.Pairing.HTTP1Test`, `Frameshift.Pairing.TLSServerTest`, `Frameshift.LocalIPC.ServerTest`, `Frameshift.SimulatorTest`, `PairingQRReaderTests`, pairing schema fixtures | Bounded QR/request parsing, physical-window state, QR image capture and confirmation, pinned host client, fail-closed simulator custody, a live mutual-TLS pair-to-authenticated-TD exchange, and transient authenticated local pairing admission pass in software. The installed shell has a no-retry pairing call; a complete installed Mac-to-independent-frame run remains open |
