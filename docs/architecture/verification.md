...
| --- | --- | --- | --- |
| Portable domain boundaries preserve delivery, content-reference, replay, and audit atomicity under one writer | Core | `Frameshift.Delivery.TransitionTest`, existing outbox/direct-delivery crash and replay tests; Mac/Linux contract tests required | Pure Delivery decisions are extracted; remaining context extraction and cross-platform qualification remain open |
...
| Console.app and `/usr/bin/log` read core and shell operational logs without a Frameshift UI; health, metrics, and audit have a bounded read-only CLI | Core/shell | `Frameshift.LocalIPC.DiagnosticsServerTest` covers a stalled client, collector outage, and unsafe socket directory; `CoreLogBridgeTests` checks public field validation; `scripts/check-packaged-app` exercises the installed CLI; bridge-failure and Linux tests required | A packaged Mac app showed native shell and core command logs; the bundled CLI read health over peer-authenticated IPC. Production signing, log backpressure, and long-running behavior remain open |
| Local metric catalog preserves units, bounded dimensions, restart coverage, and disk/resource ceilings | Core | `Frameshift.Diagnostics.CatalogTest`, `Frameshift.LocalIPC.DiagnosticsServerTest` cover bounded labels, invalid rollups, restart readback, and collector outage; long-running load and idle measurements on Mac and Linux required | Local reporter, rollups, bounded labels, restart readback, and collector status are implemented; a complete coverage interval and physical disk/resource ceilings remain unqualified |
