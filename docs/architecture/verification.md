...
| A paired frame record binds one admitted universal TD and capability instance to a pinned server SPKI fingerprint and opaque Keychain credential reference, survives restart, rejects conflicting re-admission and duplicate SPKI custody, and supplies UI targets without vendor branching | Frame Protocol/core | `Frameshift.FrameRegistryTest`, `Frameshift.LibraryTest`, `Frameshift.Outbox.EndpointTest`, `Frameshift.LocalAPITest`, and `FrameDiscoveryTests` for bounded Bonjour TXT parsing; live browse and pairing acceptance required | Idempotent post-pairing admission and unique pin custody are implemented; preexisting duplicate pins prevent migration rather than being silently reassigned. The Mac Settings view browses and bounds privacy-minimal frame introductions while visible; live discovery, physical pair mode, certificate issuance/rotation, explicit TD refresh, and Keychain writes remain open |
| A physical bootstrap record contains only a device ID, frame SPKI pin, and at least 16 secret bytes; the host checks the presented peer certificate before sending the secret and later matches the authenticated TD device ID | Pairing/core | `Frameshift.Pairing.BootstrapTest`, `Frameshift.Pairing.RequestTest`, `Frameshift.Pairing.WindowTest`, `Frameshift.Pairing.EndpointTest`, `Frameshift.Pairing.ClientTest`, `Frameshift.Pairing.AdmissionTest`, `Frameshift.Pairing.StoreTest`, `Frameshift.Pairing.HTTP1Test`, `Frameshift.Pairing.TLSServerTest`, `Frameshift.LocalIPC.ServerTest`, `Frameshift.SimulatorTest`, pairing schema fixtures | Bounded QR/request parsing, physical-window state, pinned host client, fail-closed simulator custody, a live pre-pair TLS socket, and transient authenticated local pairing admission pass in software. The installed shell has a no-retry pairing call, but QR capture, host Keychain certificate issuance, discovered-endpoint handoff, and an installed live pair-to-TD run remain open |
