...
| Public static installation guide accurately links qualified artifacts and labels simulated versus physical evidence | Guide/release | `scripts/check guide` builds and tests the static lab against the JavaScript kernel; `scripts/check guide-browser` exercises a 390 px Chrome viewport, accessible names, keyboard controls, invalid file refusal, local still, profile refusal, interrupted transfer, offline interaction, and recovery; `GuideHandoffTests` and the packaged URL registration check cover bounded native hints; manual accessibility, browser matrix, offline reload behavior, manifest links and digests, and installed handoff acceptance remain required | Local guide, Chrome smoke, and bounded native handoff tests pass; no deployed release guide, qualified download links, or installed handoff evidence |
