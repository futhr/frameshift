...
| Each accepted render and delivery intent pins an asset-specific work digest and a reusable admitted renderer/profile/transfer qualification through restart, switching, and rollback | Core/renderer/delivery | Versioned qualification/work manifests and producer fixtures, migration, pin/restart, incompatible profile, duplicate transfer, ambiguous effect, and rollback tests required | Specified in D-014; durable qualification admission, binary attestation, workflow pinning, cohort promotion, and physical qualification are open |
| The packaged macOS app presents the supplied temporary Futhr mark in the Dock and Finder, with light/dark appearance variants and the same authoritative shell controls from its Dock entry | Swift shell/package | Packaged icon resource checks and a live desktop appearance/reopen check required | Specified; asset packaging and desktop evidence are open |
