...
| Public static installation guide accurately links qualified artifacts and labels simulated versus physical evidence | Guide/release | `scripts/check guide` builds and tests the static lab against the JavaScript kernel and verifies conditional link markup; a local signed fixture build exercises link insertion and partial-input refusal; `scripts/check guide-browser` exercises a 390 px Chrome viewport, accessible names, keyboard controls, invalid file refusal, local still, profile refusal, interrupted transfer, offline interaction, and recovery; `GuideHandoffTests` and the packaged URL registration check cover bounded native hints; manual accessibility, browser matrix, offline reload behavior, public manifest links and digests, and installed handoff acceptance remain required | Local guide, signed fixture build, Chrome smoke, and bounded native handoff tests pass; no deployed release guide, qualified public downloads, or installed handoff evidence |
| Release links identify signed exact archive bytes before publication | Release | `scripts/check release` exercises detached Ed25519 verification, strict schema, HTTPS/versioned URL, regular-file size/digest, tamper, trusted-key refusal, bounded HTTPS redirect, partial/compressed response refusal, and public byte verification; signed real artifacts and publication acceptance required | Local and synthetic public-response verifiers pass; owner-pinned production key, signed artifacts, real public URL readback, and distribution remain open |
...
| Console.app and `/usr/bin/log` read core and shell operational logs without a Frameshift UI; health, metrics, and audit have a bounded read-only CLI | Core/shell | `Frameshift.LocalIPC.DiagnosticsServerTest` covers a stalled client, collector outage, and symlinked socket directory; `Frameshift.LocalIPC.ServerTest` covers symlink refusal and a second listener against a live socket; `CoreLogBridgeTests` checks public field validation and bounded queue saturation; `scripts/check-packaged-app` exercises all three installed CLI reads; bridge-failure and Linux tests required | A packaged Mac app showed native shell and core command logs; the bundled CLI read health, metrics, and audit over peer-authenticated IPC. Both listeners now inspect their directory before changing permissions. Core log emission has a 512-record queue with counted loss; production signing, bridge-failure behavior, and long-running resource behavior remain open |
