...
| Each accepted render and delivery intent pins one qualified asset/profile, renderer build, and transfer generation through restart, switching, and rollback | Core/renderer/delivery | Versioned manifest and producer fixtures, migration, pin/restart, incompatible profile, duplicate transfer, ambiguous effect, and rollback tests required | Specified in D-014; durable generation admission, binary attestation, workflow pinning, cohort promotion, and physical qualification are open |
