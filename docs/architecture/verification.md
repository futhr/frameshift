...
| Each accepted render and delivery intent pins an asset-specific work digest and a reusable admitted renderer/profile/transfer qualification through restart, switching, and rollback | Core/renderer/delivery | `Frameshift.Qualification.IdentityTest`, `Frameshift.Qualification.ProfileTest`, and `Frameshift.Qualification.StoreTest` cover canonical identity, profile changes, durable candidate/admission/activation, work/result custody, replay, restart, switch, rollback, and refusal of mismatched recipes; `Frameshift.RendererTest` checks the startup executable digest. Producer fixtures, legacy migration, delivery pin/restart, duplicate transfer, and ambiguous effect tests required | Pure identities, startup renderer fingerprint, durable binding decisions, and atomic accepted work/result custody are implemented; exact launched-process attestation, render/delivery workflow pinning, cohort promotion, and physical qualification are open |
