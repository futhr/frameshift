...
| Console.app and `/usr/bin/log` read core and shell operational logs without a Frameshift UI; health, metrics, and audit have a bounded read-only CLI | Core/shell | `Frameshift.LocalIPC.DiagnosticsServerTest`, `Frameshift.Diagnostics.LogFormatterTest`, `scripts/check-packaged-app`; additional bridge-failure and Linux tests required | A packaged Mac app showed native shell and core command logs; the bundled CLI read health over peer-authenticated IPC. Production signing and long-running behavior remain open |
| Local metric catalog preserves units, bounded dimensions, restart coverage, and disk/resource ceilings | Core | `Frameshift.Diagnostics.CatalogTest`, `Frameshift.LocalIPC.DiagnosticsServerTest`; long-running load and idle measurements on Mac and Linux required | Local reporter, rollups, bounded labels, restart readback, and collector coverage status are implemented; physical disk and resource ceilings remain unqualified |
