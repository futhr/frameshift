...
- **Decision:** Local generation is preferred. Draw Things/
  ...
  Things+ is a qualified-cloud candidate to evaluate. Gemini image models are
  optional metered API providers or manual-import sources.
- **Consequence:** Consumer web subscriptions are never automated or presented
  as API entitlement. Cloud use requires an explicit destination and usage
...
## D-019 — Independent guide first, optional external actions last

- **State:** required order retained; integration scope revised 2026-09-25
- **Decision:** Complete the visual configurator and exact printable list in D,
  and research/operational qualification in E. Optional provider/financial
  actions, purchasing, fulfillment and care are final F after D/E.
...
- **Decision:** Use one authorized, bounded Beamlens supervisor. Configure its
  ...
  and diagnostics do not automatically share a registry. Use the selected
  GreptimeDB/structured telemetry path without ELK in the operated host.
...
- **Decision:** Frameshift remains a complete product. Its physical BuildSpec
  becomes a versioned product profile of qualified Conjunct composition
...
- **Consequence:** No competing encoders, UI graph, effect journal or copied
  ...
  accepted identities through migration. Current/unpushed paths must be read
  before restructuring. Private corporate strategy, monetization and legal
  analysis remain outside this repository.
- **Detail:** [Conjunct adoption](../research/conjunct-adoption.md),
